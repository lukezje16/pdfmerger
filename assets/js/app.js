...
(function($) {
    ...
    function updateMergeButton() {
        const canMerge = State.files.length >= 2 && !State.isUploading && !State.isMerging;
    ...
    function updateFileOrder() {
        Elements.fileList.find('.file-item').each(function(index) {
            $(this).find('.file-order').text(index + 1);
        });
    }
    ...
    function createFileItemHTML(file, order) {
        return `
            <li class="file-item" data-id="${file.id}" data-server-id="${file.serverId || ''}">
    ...
    async function handleFiles(fileList) {
        ...
        for (const file of validFiles) {
            ...
            $item.find('.file-icon i').removeClass('bi-file-earmark-pdf-fill').addClass('bi-arrow-repeat');
            ...
            try {
                ...
                $item.find('.file-icon i').removeClass('bi-arrow-repeat').addClass('bi-file-earmark-pdf-fill');
    ...
    async function mergePDFs() {
        ...
        try {
            ...
            if (response.success) {
                ...
                // Auto-download
                const downloadUrl = Config.downloadEndpoint + '?id=' + response.downloadId;
